 ...
 // mapi_var_dump($field);
	...
	if($field["type"] == 'creditcard' ) {
		...
		foreach($field["inputs"] as $key => $thefield ){
			...
			$content = str_replace('ginput_card_expiration_month', 'ginput_card_expiration_month card_month form-control  col-xs-6', $content);
			$content = str_replace('ginput_card_expiration_year', 'ginput_card_expiration_year card_year form-control  col-xs-6', $content);
	...
	//wp_enqueue_style( 'anagram_gforms-style', get_stylesheet_directory_uri().'/css/gforms-style.css', array(), filemtime( get_stylesheet_directory().'/css/gforms-style.css') );
		...
		foreach ( $form['fields'] as $field ) {
			...
			if ( $field->type == 'creditcard' ) {
				...
				wp_enqueue_script('anagram_card-custom', (get_template_directory_uri()."/js/card-gravity.js"),'jquery',filemtime( get_stylesheet_directory().'/js/card-gravity.js'),false);

			}
		}

}
//NOT used Note: this will allow for the labels to be used during the submission process in case values are enabled
