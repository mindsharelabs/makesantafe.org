...
*/
				...
				//user sumitted photos
				if($_post->post_parent == 4894 )$image_title = 'Maker: '.get_the_author_meta( 'display_name', $_post->post_author );

					//htmlspecialchars('<strong>'.$image_title .'</strong><br/> '.$image_caption)
				$output_buffer .='
				<div class="item '.$args['itemclass'].'">
					<a href="'. $full[0] .'" itemprop="contentUrl" rel="lightbox" title="'.htmlspecialchars($image_title ).'" data-size="'.$full[1].'x'.$full[2].'">
