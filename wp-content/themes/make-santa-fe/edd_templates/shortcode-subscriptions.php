		...
		<table id="edd_user_history">
			...
			<?php foreach ( $subscriptions as $subscription ) :
				...
				<tr>
					...
					<td>
						<a href="<?php echo esc_url( add_query_arg( 'payment_key', edd_get_payment_key( $subscription->parent_payment_id ), edd_get_success_page_uri() ) ); ?>" class="edd_subscription_invoice"><?php _e( 'View Invoice', 'edd-reccuring' ); ?></a>
			...
			<?php endforeach; ?>
								...
								<?php
								$meta    = edd_get_payment_meta( $subscription->parent_payment_id  );
							//custom monthly dicount code
							if((strtotime( $meta['date'] ) <= strtotime( '2016-03-18 00:00:01' ) ) && $frequency=='Monthly' ){
							 echo '<em>You will only be billed <strong>$50</strong> a month till April 1st, 2017 as a Founding Member.</em>';
							};
