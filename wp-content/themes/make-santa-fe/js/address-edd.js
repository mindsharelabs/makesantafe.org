...
*/
 ...
 jQuery(function($) {
	    ...
	    jQuery('#edd_cc_address').find('span:first').after('<div id="geocomplete_holder" class="form-group"><input id="geocomplete" tabindex="1013" class="form-control" type="text" placeholder="Populate fields from your street address or enter them below" value="" /><span id="geo_error" class="help-block"></span></div>');

	    //jQuery('#edd_cc_address p').hide();
         ...
         // types: ["geocode", "establishment"],
			...
			jQuery('#geo_error').text('');
			  ...
			  if( typeof sNumber !== 'undefined' ){
	         	 jQuery('#card_address').val(sNumber+' '+address);
				 jQuery('#card_city').val(city);

				 //jQuery('#billing_country').val(country).change();
				 jQuery('#card_state').val(province).change();
				 jQuery('.card-zip').val(zip);
				 jQuery('#geocomplete_holder').hide();
				 jQuery('#edd_cc_address p').slideDown();
			}else{
