	...
	class Pie_WCWL_Waitlist_Settings {
		...
		public function enqueue_settings_scripts( $hook ) {
			if ( 'woocommerce_page_wc-settings' == $hook && $this->is_waitlist_settings_page() ) {
				wp_enqueue_script( 'wcwl_admin_settings', WCWL_ENQUEUE_PATH . '/includes/js/wcwl_admin_settings.js', array(), WCWL_VERSION );
				wp_localize_script( 'wcwl_admin_settings', 'wcwl_settings', $this->get_script_data() );
				wp_enqueue_style( 'wcwl_admin_settings', WCWL_ENQUEUE_PATH . '/includes/css/wcwl_admin_settings.css', array(), WCWL_VERSION );
			}
		}
		 ...
		 * @return bool
		 */
		private function is_waitlist_settings_page() {
		...
		private function get_script_data() {
			return array(
				...
				'update_warning'                 => sprintf( __( '%sWarning:%s This could take a long while depending on how many products there are. Do not navigate away from this page until the update is complete.', 'woocommerce-waitlist' ), '<b>', '</b>' ),
		...
		public function get_settings() {
			$settings = array(
				array( 'title' => 'Waitlist Display Options', 'type' => 'title', 'id' => 'waitlist_display' ),
				array(
				...
				array( 'type' => 'sectionend', 'id' => 'waitlist_updates' ),
				array( 'title' => 'Waitlist Admin Options', 'type' => 'title', 'id' => 'waitlist_admin' ),
				array(
				...
				array(
					...
					'id'      => 'woocommerce_waitlist_minimum_stock',
					'default' => 1,
					'type'    => 'number',
				),
				array(
					'title'   => __( 'Notify admin user on new waitlist signup', 'woocommerce-waitlist' ),
					'desc'    => __( 'Send an notification email to a site administrator when a user registers to a waitlist', 'woocommerce-waitlist' ),
					'id'      => 'woocommerce_waitlist_notify_admin',
					'default' => 'no',
					'type'    => 'checkbox',
				),
				array(
					'title' => __( 'Administrator\'s email address for notifications', 'woocommerce-waitlist' ),
					'desc'  => sprintf( __( 'If no email address is entered the stored site administrator email (%s) will be used', 'woocommerce-waitlist' ), get_option( 'admin_email' ) ),
					'id'    => 'woocommerce_waitlist_admin_email',
					'type'  => 'email',
				),
				array( 'type' => 'sectionend', 'id' => 'waitlist_admin' ),
				array(
		...
		public function filter_new_account_email_description( $translated_text ) {
			if ( 'Customer "new account" emails are sent to the customer when a customer signs up via checkout or account pages.' == $translated_text ||
				 'Customer new account emails are sent when a customer signs up via the checkout or My Account page.' == $translated_text ) {
