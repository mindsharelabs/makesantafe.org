...
if ( ! class_exists( 'Pie_WCWL_Frontend_User_Waitlist' ) ) {
	...
	class Pie_WCWL_Frontend_User_Waitlist {
		...
		public function __construct() {
			...
			$this->products = WooCommerce_Waitlist_Plugin::get_waitlist_products_by_user_id( $this->user->ID );
			$this->archives = WooCommerce_Waitlist_Plugin::get_waitlist_archives_by_user_id( $this->user->ID );
			$this->init();
		}
		...
		public function display_users_waitlists() {
			...
			Pie_WCWL_Compatibility::wcwl_get_template( 'waitlist-user-waitlist.php', array(
				'title'    => $this->waitlist_tab_title,
		...
		public function init() {
			...
			if ( isset( $_GET['remove_waitlist'] ) && is_numeric( $_GET['remove_waitlist'] ) ) {
				wc_add_notice( __( 'You have been removed from the waitlist for this product', 'woocommerce-waitlist' ) );
			}
			if ( isset( $_GET['remove_archives'] ) && 'true' === $_GET['remove_archives'] ) {
		...
		public function enqueue_scripts() {
			wp_enqueue_script( 'wcwl_frontend_account', WCWL_ENQUEUE_PATH . '/includes/js/wcwl_account.js', array(), WCWL_VERSION, true );
			$data = array(
		...
		/**
		 * Setup text strings required for user waitlists on frontend
		 */
		private function setup_text_strings() {
			$this->waitlist_tab_title = __( 'Your Waitlists', 'woocommerce-waitlist' );
		}
	}
}