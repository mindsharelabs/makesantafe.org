...
if ( ! class_exists( 'Pie_WCWL_Frontend_Simple' ) ) {
	...
	class Pie_WCWL_Frontend_Simple extends Pie_WCWL_Frontend_Product {
		...
		private function init() {
			...
			if ( $this->waitlist_is_enabled_for_product( $this->product_id ) ) {
		...
		private function get_waitlist_elements_for_logged_out_user( $string ) {
			$url = $this->create_button_url( 'join', $this->product_id );
		...
		private function get_waitlist_control( $context ) {
			...
			$url         = $this->create_button_url( $context, $this->product_id );
		...
		public function append_waitlist_message( $array, $product ) {
			if ( ! $this->user || ! $this->product->waitlist->user_is_registered( $this->user->ID ) ) {
